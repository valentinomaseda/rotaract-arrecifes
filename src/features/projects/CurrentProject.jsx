...
const ProjectCard = ({ badge, title, description, stats, cta, active, autoplayKey, autoplayDuration, paused }) => (
  <div
    className={`flex flex-col h-full bg-gradient-to-b from-gray-50 via-white to-gray-50/50 rounded-3xl overflow-hidden border border-gray-100 shadow-xl transition-all duration-500 ${active
          ...
          <div className="flex items-center justify-center gap-2 mt-5" role="tablist" aria-label="Proyectos">
            {PROJECTS.map((p, idx) => (
              <button
                ...
                onClick={() => goTo(idx)}
                className={`rounded-full transition-all duration-300 focus-visible:outline-2 focus-visible:outline-cranberry ${idx === current
