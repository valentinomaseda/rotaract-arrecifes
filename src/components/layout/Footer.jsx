...
const navLinks = [
  ...
  { href: '#past-projects-title', label: 'Proyectos' },
  { href: '#unete', label: 'Voluntariado' },
];
...
const contactItems = [
  {
    ...
    text: 'Calle Falsa 123, Arrecifes',
  },
  {
    icon: (
    ...
    <footer className="bg-gray-900 text-white" aria-label="Pie de página">
      ...
      <div className="max-w-7xl mx-auto py-16 px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 lg:gap-16">
            ...
            <div className="flex gap-3 pt-1">
              {socialLinks.map(({ name, href, icon }) => (
                <a
                  key={name}
                  href={href}
                  className="w-9 h-9 rounded-full bg-white/8 text-gray-400 hover:bg-cranberry hover:text-white flex items-center justify-center transition-all duration-300 hover:scale-110"
                  aria-label={name}
                >
                  {icon}
                </a>
          ...
          {/* Nav links */}
          <div>
            <h3 className="text-xs font-montserrat font-semibold text-gray-500 tracking-[0.15em] uppercase mb-5">
              Navegación
            </h3>
            ...
            <ul className="space-y-4">
              {contactItems.map(({ icon, text }) => (
                <li key={text} className="flex items-start gap-3 text-gray-400 font-montserrat text-sm">
                  <span className="text-cranberry mt-0.5">{icon}</span>
                  {text}
                </li>
              ))}
            </ul>
